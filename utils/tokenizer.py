...
class GloveTokenizer(Tokenizer):
    ...
    def build_embedding(self, fname, emb_dim=300, save_pkl=True, dataset_name='',voc_size=None, word2id=None, id2word=None):
        # get embedding
        import io, os
        ...
        def load_vectors(fname):
            ...
            for line in tqdm(f.readlines(), total=2196017):
                ...
                try:
                    if len(values[1:]) == 300:
                        vectors = [ float(x) for x in values[1:] ]
                        embedding = np.asarray(vectors, dtype='float32')
